"""
This module contains tests for the pulp_npm.extensions.admin.cudl module.
"""
import unittest
...
from pulp_npm.extensions.admin import cudl


class TestCreateNpmRepositoryCommand(unittest.TestCase):
    @mock.patch('pulp_npm.extensions.admin.cudl.CreateAndConfigureRepositoryCommand.__init__')
    @mock.patch('pulp_npm.extensions.admin.cudl.ImporterConfigMixin.__init__')
    @mock.patch('pulp_npm.extensions.admin.cudl.NpmRepositoryOptions.__init__')
    def test___init__(self, pro___init__, icm___init__, cacrc___init__):
        ...
        cprc = cudl.CreateNpmRepositoryCommand(context)
    ...
    def test_default_notes(self):
        ...
        self.assertEqual(cudl.CreateNpmRepositoryCommand.default_notes.get(REPO_NOTE_TYPE_KEY),
                         constants.REPO_NOTE_NPM)

    def test_importer_id(self):
        # this value is required to be set, so just make sure it's correct
        self.assertEqual(cudl.CreateNpmRepositoryCommand.IMPORTER_TYPE_ID,
                         constants.IMPORTER_TYPE_ID)


class TestNpmRespositoryOptions(unittest.TestCase):
    """
    Test the NpmRespositoryOptions mixin class.
    """
    class MixinTestClass(cudl.NpmRepositoryOptions):
        """
        NpmRepositoryOptions depends on self having add_option() and self.options_bundle,
        ...
        def __init__(self):
            ...
            super(TestNpmRespositoryOptions.MixinTestClass, self).__init__()

    def test___init__(self):
        """
        Assert correct behavior from the __init__() method.
        """
        pro = TestNpmRespositoryOptions.MixinTestClass()
    ...
    @mock.patch('pulp_npm.extensions.admin.cudl.NpmRepositoryOptions.parse_user_input',
                create=True)
...
class TestUpdateNpmRepositoryCommand(unittest.TestCase):

    def setUp(self):
        self.context = mock.Mock()
        self.context.config = {'output': {'poll_frequency_in_seconds': 3}}
        self.command = cudl.UpdateNpmRepositoryCommand(self.context)
    ...
    @mock.patch('pulp_npm.extensions.admin.cudl.ImporterConfigMixin.__init__')
    @mock.patch('pulp_npm.extensions.admin.cudl.NpmRepositoryOptions.__init__')
    @mock.patch('pulp_npm.extensions.admin.cudl.UpdateRepositoryCommand.__init__')
    def test___init__(self, urc___init__, pro___init__, icm___init__):
        ...
        cprc = cudl.UpdateNpmRepositoryCommand(context)
...
class TestListNpmRepositoriesCommand(unittest.TestCase):
    def setUp(self):
        self.context = mock.Mock()
        self.context.config = {'output': {'poll_frequency_in_seconds': 3}}
    ...
    def test_get_all_repos(self):
        ...
        command = cudl.ListNpmRepositoriesCommand(self.context)
    ...
    def test_get_repositories(self):
        ...
        repos = [
            {
                'id': 'matching',
                'notes': {REPO_NOTE_TYPE_KEY: constants.REPO_NOTE_NPM, },
                'importers': [
                    {'config': {}}
                ],
                'distributors': [
    ...
    def test_get_other_repositories(self):
        ...
        # Verify
        self.assertEqual(1, len(repos))
        self.assertEqual(repos[0]['repo_id'], 'non-npm-repo-1')
