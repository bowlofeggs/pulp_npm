...
from pulp_npm.plugins.distributors.steps import NpmPublisher

PLUGIN_DEFAULT_CONFIG = {
    constants.CONFIG_KEY_PUBLISH_DIRECTORY: constants.CONFIG_VALUE_PUBLISH_DIRECTORY,
    constants.CONFIG_KEY_PUBLISH_DOMAIN: constants.CONFIG_VALUE_PUBLISH_DOMAIN
}
...
def entry_point():
    ...
    return NpmDistributor, plugin_config


class NpmDistributor(Distributor):
    """
    ...
    def metadata(cls):
        ...
        * types        - List of all content type IDs that may be published using this
        ...
        return {
            ...
            'display_name': _('Npm Distributor'),
            'types': [constants.PACKAGE_TYPE_ID]
        }

    def __init__(self):
        """
        Initialize the NpmDistributor.
        """
        super(NpmDistributor, self).__init__()
    ...
    def publish_repo(self, repo, publish_conduit, config):
        ...
        """
        _logger.debug('Publishing Npm repository: %s' % repo.id)
        self._publisher = NpmPublisher(repo, publish_conduit, config)
    ...
    def cancel_publish_repo(self):
        ...
        """
        _logger.debug('Canceling Npm repository publish')
        self.canceled = True
