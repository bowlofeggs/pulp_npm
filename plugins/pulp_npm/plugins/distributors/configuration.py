...
def get_web_publish_domain(config):
    # TODO Add Doc string
    return config.get(constants.CONFIG_KEY_PUBLISH_DOMAIN)


def get_root_publish_directory(config):
    """
    The publish directory for the Npm plugin.

    :param config: Pulp configuration for the distributor
    :type  config: pulp.plugins.config.PluginCallConfiguration
    :return:       The publish directory for the Npm plugin
    :rtype:        str
    """
